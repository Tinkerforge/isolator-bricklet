...
use tinkerforge::{ipconnection::IpConnection, isolator_bricklet::*};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let isolator_bricklet = IsolatorBricklet::new(UID, &ipcon); // Create device object

    ipcon.connect(HOST, PORT).recv()??; // Connect to brickd
                                        // Don't use device before ipcon is connected

    // Get current statistics
    let get_statistics_result = isolator_bricklet.get_statistics().recv()?;
